...
class Solution:
	...
	def get_strip_sum(self, A, B):
		strip_sum = [[0 for j in range(len(A))] for i in range(len(A))]
		for i in range(len(A)):
			for j in range(len(A)):
				if i < 1:
					strip_sum[i][j] = A[i][j]
				elif i < B:
					strip_sum[i][j] = A[i][j] + strip_sum[i-1][j]
				else:
					strip_sum[i][j] = A[i][j] + strip_sum[i-1][j] - A[i-B][j]

		return strip_sum

	def extract_max_sum(self, strip_sum, B):
		max_sum = -sys.maxsize
		for row in range(len(strip_sum)):
			sum = 0
			for col in range(len(strip_sum)):
				if col < B:
					sum  = sum + strip_sum[row][col]
				else:
					sum = sum - strip_sum[row][col-B] + strip_sum[row][col]
	...
	def solve(self, A, B):
		max_sum = -sys.maxsize
		strip_sum = self.get_strip_sum(A, B)
		max_sum = self.extract_max_sum(strip_sum, B)
		return max_sum		 
